...
task :passenger_start => :environment do
  queue "cd #{deploy_to}/#{current_path} && /usr/local/rvm/gems/ruby-2.0.0-p481/bin/passenger start -a 0.0.0.0 -p #{ENV['port']} -d -e production --pid-file #{deploy_to}/#{shared_path}/passenger.#{ENV['port']}.pid"
end

task :passenger_stop => :environment do
  quene "touch #{deploy_to}/#{current_path}/passenger.#{ENV['port']}.pid"
  queue "cd #{deploy_to}/#{current_path} && /usr/local/rvm/gems/ruby-2.0.0-p481/bin/passenger stop -p #{ENV['port']} --pid-file #{deploy_to}/#{shared_path}/passenger.#{ENV['port']}.pid"
end
