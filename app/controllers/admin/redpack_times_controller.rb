class Admin::RedpackTimesController < Admin::BaseController
  ...
  def create
    ...
    for i in 1..total
      TimeAmount.create(:time => @redpack_time.start_time+@redpack_time.frequency*i*60, :time_end => @redpack_time.start_time+@redpack_time.frequency*i*60+5*60, :amount => @redpack_time.amount, :redpack_time_id => @redpack_time.id)
    end
